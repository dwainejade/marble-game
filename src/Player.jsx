...
export default function Player() {
  ...
  useEffect(() => {
    const unsubscribePhase = useGame.subscribe(
      ...
      (value) => {
        if (value === "ready") {
          console.log("Phase is now: ", value);
        }
      }
    );
  ...
  useFrame((state, delta) => {
    ...
    // Phases
    if (bodyPosition.z < -(blocksCount * 4 + 2)) useGame.getState().end();

    if (bodyPosition.y < -4) useGame.getState().restart();
  });
